...
| D5 | Vanilla fallback handler duplicated verbatim between `demo/index.html` and the Lit elements | copy-paste, two homes | any fallback bugfix must be applied twice or drifts | ✅ repaid **this epic** (`src/fallback/**` → `dist/semtext-fallback.js`; the demo page now carries a build marker, not a hand-written IIFE) | correctness |
