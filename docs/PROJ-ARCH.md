...
`spec/` precede implementation code, with the Lit `SemNote` component +
...
```mermaid
graph LR
    A[Author] -->|writes| S["spec/ specs (conventions.md, schema/sem-note.md)"]
    ...
    C["web/demo/index.html"] -.reference impl.-> D
...
| Component | Purpose |
|-----------|---------|
| Tier-0 specs (`spec/`) | Authoring conventions (v0.4 class-based baseline) + `sem-note` element schema; BDD source of truth |
...
| Standalone demo (`web/demo/index.html`) | Reference implementation of the v0.4 baseline (single file: core CSS + Tailwind `@apply` layer + fallback JS) |
| Cypress e2e (`test/`) | Validates demo behavior; mirrors the `sem-note` schema BDD |
...
- **Spec → spec → code change order**: `spec/schema/sem-note.md` precedes conventions precedes implementation precedes cypress.
